    ...
    <div className="flex flex-col gap-2">
      ...
      <Listbox value={value} onChange={onChange}>
        <div className={clsx('relative', disabled && 'cursor-not-allowed')}>
          <Listbox.Button className="has relative h-[44px] w-full  cursor-default rounded-[5px] border-2 border-grey-4 px-3 py-2 text-left text-sm aria-expanded:border-green-1 ">
            {value ? (
              <span className="block truncate">
                {options.find((option) => option.value === value)?.label}
              </span>
            ) : (
              <span className="block truncate pr-7 text-grey-1">
                {placeholder}
              </span>
            )}
