...
const RegisterPage = ({}) => {
  ...
  const handleGetProvinces= async () => { 
    try {
      ...
      if(response.status === 200) {
        setProvinceList(response.data.data.map(province => ({ label: province.name, value: province.id })))
      }
    } catch (error) {
      toast.error(error)
    }
  ...
  const handleOnProvinceChange = async (id) => {
    try {
      ...
      const response = await getCities({ params: { province_id: id } })
      if(response.status === 200) {
        setCityList(response.data.data.map(city => ({ label: city.name, value: city.id })))
      }
    } catch (error) {
      toast.error(error)
    }
  ...
  const handleRegister = async () => { 
    try {
      ...
      if(response.status === 200) {
        toast.success('Pendaftaran Berhasil. Terima kasih atas partisipasinya, kami tunggu di acara!')
        resetState()
      }
    } catch (error) {
      toast.error(error);
  ...
  const totalFee = selectedValues.reduce((sum, value) => sum + value.fee_nominal, 0);

  useEffect(() => {
    handleGetEvents();
    handleGetProvinces()
  }, []);
  return <div className="relative min-h-[100vh] flex items-center">
          <img src="/image/background/bg-register.png" className="absolute bottom-0 z-10" alt="" />
          <img src="/image/background/bg-register.png" className="absolute right-0 top-0 rotate-180 z-10" alt="" />
      ...
      <div className="container z-20 py-44 md:py-0">
          ...
          <h1 className="text-2xl md:text-5xl font-semibold mt-12">Form Pendaftaran SAN Main Event</h1>
          <div className="flex flex-col md:flex-row gap-[70px] mt-[30px]">
            <div className="flex flex-col w-full md:w-1/2 gap-6">
                <MultipleCheck options={eventList} selectedValues={selectedValues} onChange={setSelectedValues} label="Ingin ikut event apa?" />
                <Input label="Nama " inputProps={
                    {
                ...
                onInput={(value) => setForm({...form, email: value})}/>
                 <Input label="Nomor Telepon" inputProps={
                    {
            ...
            <div className="w-full md:w-1/2 flex flex-col gap-6">
              <Select placeholder="Pilih Provinsi" value={selectedProvince} label={'Lokasi'}  options={provinceList}  onChange={(value) => handleOnProvinceChange(value)}/>
              <Select placeholder="Pilih Kota" value={form.city_id} label={'Lokasi'}  options={cityList}  onChange={(value) => setForm({...form, city_id: value})}/>
              <div className="flex flex-col">
                  ...
                  <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-4">
                        <Image src={'/image/logo/logo-bca.png'} width={56} height={18} alt="bca" />
                        <span className="text-[22px] font-semibold">1984673940378</span>
                      </div>
                  </div>  
              </div>
              <UploadFile value={selectedFile} onChange={(e) => {setSelectedFile(e.target.files[0])}} />
